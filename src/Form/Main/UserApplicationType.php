...
class UserApplicationType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) use ($builder) {
            ...
            if ($userApplication && $application = $userApplication->getApplication()) {
                $form
...
//                    'prototype' => true,
                    ...
                    ->add('application', EntityType::class, array(
                        ...
                        'query_builder' => function (EntityRepository $er) use ($builder) {
                            $subq = $er->createQueryBuilder('sa')
                                ->innerJoin('\App\Entity\Main\UserApplication', 'ua', 'WITH', 'ua.application = sa.id')
                                ->where('ua.user = ?1');
            ...
            $form->add('submit', Types\SubmitType::class, array(
                'label' => 'save',
                'attr' => array('class' => 'btn btn-inline  btn-primary col-md-3'),

            ))
                ->add('cancel', Types\ButtonType::class, array(
                    'label' => 'cancel',
                    'attr' => array('class' => 'btn btn-secondary col-md-3', 'onclick' => $this->cancel_url),
